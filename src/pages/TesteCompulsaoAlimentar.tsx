...
const TesteCompulsaoAlimentar = () => {
  ...
  const handleEmailSubmit = (data: { email: string; whatsapp: string }) => { setUserEmail(data.email); setUserWhatsapp(data.whatsapp); setCurrentStep("questions"); setCurrentQuestion(0); setAnswers([]); setSelectedAnswer(null); };
  ...
  const handleAnswer = () => {
    ...
    else { const total = newAnswers.reduce((sum, score) => sum + score, 0); setFinalScore(total); setCurrentStep("results"); submitTestResult({ email: userEmail, whatsapp: userWhatsapp, testType: "Compulsão Alimentar", score: total, maxScore: questions.length * 3, resultLevel: total > 25 ? "Positivo" : "Negativo", answers: newAnswers }); }
  };
